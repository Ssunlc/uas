  ...
  methods: {
    register (formName) {
      ...
      that.$refs[formName].validate((valid) => {
        if (valid) {
          // 发送注册请求
          if(that.sendRegisterRequest(formName)){
            // 注册成功后进行页面跳转
            api.reqSuccess(that, systemConfig.tips.register.userRegisterSuccess)
            var url = '/index'
            that.$router.push(url)
          }
          api.reqFail(that, systemConfig.tips.register.userRegisterError)
        } else {
    ...
    sendRegisterRequest (formName) {
      const that = this
      switch (formName) {
        case 'userForm':
          return that.userRegister(formName)
          break
      }
    },
    userRegister (formName) {
      const that = this
      var params = {}
      params.url = `${systemConfig.requestUrlConfig.register.userRegister}`
      params.datas = {}
      params.datas.captcha = that.$refs[formName].captcha
      params.datas.password = that.$refs[formName].password
      params.datas.nickname = that.$refs[formName].nickname
      params.datas.username = that.$refs[formName].username
      params.datas.passwordConfirm = that.$refs[formName].passwordConfirm
      params.datas.email = that.$refs[formName].email
      params.datas.phone = that.$refs[formName].phone
      api.post(params)
      .then(res => {
        const data = res.data
        if(data.code !== '000000'){
